...
fn basics_nested_types() {
	...
	let arg: Vec<PyObject> = vec![
    	a.into_object(),
    	b.into_object()
    	];
	let answ = basics.generic2(py, arg);
	assert_eq!(answ.len(), 1);
	assert_eq!(&answ.get(0).unwrap().extract::<String>(py).unwrap(), "success");
    ...
    let arg: Vec<((String, bool), PyObject)> = vec![
    	((String::from("first"), true), a1.into_object()),
